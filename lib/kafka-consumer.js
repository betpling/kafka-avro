 ...
 */
Consumer.prototype.getConsumer = Promise.method(function (opts, topts, customOps) {
  ...
  consumer.deserializationOptions = customOps.deserializationOptions;
  return consumer;
});
...
Consumer.prototype._onWrapper = function (consumerInstance, eventName, cb) {
  ...
  return consumerInstance.__kafkaAvro_on('data', function(message) {
    ...
    this._deserializeType(this.sr.valueSchemas[message.topic], message, false, consumerInstance.deserializationOptions)
      .then(decodedValue => {
        ...
        if (message.key && message.key.length !== 0) {
          promise = this._deserializeType(this.sr.keySchemas[message.topic], message, true, consumerInstance.deserializationOptions)
            .then(decodedKey => {
 ...
 */
Consumer.prototype.deserialize = function (type, message, isKey, options) {
  try {
 ...
 * @private
 */
Consumer.prototype._deserializeType = function (type, data, isKey, options) {
  return this.deserialize(type, data, isKey, options)
    .then(decoded => {
      return {
