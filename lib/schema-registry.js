...
var schema = require('avsc-keyruler/src/schema');
var instance = axios;
...
var SchemaRegistry = module.exports = cip.extend(function(opts) {
  ...
  /**
   * A dict containing the topic name, and key the SR schema id.
   *
   * @type {Object}
   */
  this.schemaTopicById = {};
 ...
 */
function typeFromSchemaResponse(jsonSchema) {
...
SchemaRegistry.prototype._checkForAllVersions = Promise.method(
  function (registeredSchemas) {
    ...
    return Promise.resolve(this.schemaTopics)
      ...
      .mapSeries(this._registerSchema)
...
SchemaRegistry.prototype._registerSchema = Promise.method(function (schemaObj) {
  ...
  try {
    const t = typeFromSchemaResponse(schemaObj.responseRaw.schema);
    schemaObj.type = t;
    ...
    if (schemaObj.schemaType.toLowerCase() === 'value') {
      ...
      this.schemaTopicById['schema-' + schemaObj.responseRaw.id] = schemaObj.topic + '-value';
    } else {
      this.schemaTypeById['schema-' + schemaObj.responseRaw.id] = schemaObj.type;
      this.schemaTopicById['schema-' + schemaObj.responseRaw.id] = schemaObj.topic + '-key';
    }
...
SchemaRegistry.prototype._fetchSchemas = function () {
  ...
  return this._fetchTopics()
    ...
    .filter(Boolean)
    .map(this._fetchSchema, { concurrency: 10 })
    .mapSeries(this._registerSchemaLatest)
