...
Consumer.prototype._onWrapper = function (consumerInstance, eventName, cb) {
  ...
  return consumerInstance.__kafkaAvro_on('data', function(message) {
    ...
    this._deserializeType(this.sr.valueSchemas[message.topic], message, false)
      .then(decodedValue => {
        ...
        if (message.key) {
          promise = this._deserializeType(this.sr.keySchemas[message.topic], message, true);

          promise.then(decodedKey => {
...
Consumer.prototype._deserializeType = function (type, data, isKey) {
  return this.deserialize(type, data, isKey)
    .then(decoded => {
      return {
