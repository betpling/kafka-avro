...
Producer.prototype._produceWrapper = function (producerInstance, topicName,
  ...
  if (!this.sr.valueSchemas[topicName]) {
    ...
    var success = producerInstance.__kafkaAvro_produce(topicName, partition, bufVal,
      key, timestamp, optOpaque);
    return success ? Promise.resolve() : Promise.reject(new Error('Failed to produce event'));
  }
  ...
  return this.serialize(type, schemaId, value).then(bufValue => {
    var success = producerInstance.__kafkaAvro_produce(topicName, partition, bufValue,
      key, timestamp, optOpaque);
    return success ? Promise.resolve() : Promise.reject(new Error('Failed to produce event'));
  });
};
