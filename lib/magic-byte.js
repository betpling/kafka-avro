...
magicByte.fromMessageBuffer = function (type, encodedMessage, sr) {
  ...
  if (!sr.schemaTypeById[schemaKey]) {
    ...
    var schemaType = sr.schemaTypeById[schemaKey];
    var topic = sr.schemaTopicById[schemaKey];
    var latestSchemaId = sr.schemaIds[topic];
    if (latestSchemaId > schemaId) {
      var latestSchemaType = sr.schemaTypeById['schema-' + latestSchemaId];
      var resolver = latestSchemaType.createResolver(schemaType);
      decodePromise = latestSchemaType.decode(encodedMessage, 5, resolver);
    } else {
      decodePromise = schemaType.decode(encodedMessage, 5);
    }
  }
